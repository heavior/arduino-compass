...
/**
	...
	@since v0.1;
**/
void QMC5883LCompass::init(int pinSDA,int pinSCL){
	swWire.begin(pinSDA, pinSCL);
	_writeReg(0x0B,0x01);
...
void QMC5883LCompass::_writeReg(byte r, byte v){
	swWire.beginTransmission(_ADDR);
	swWire.write(r);
	swWire.write(v);
...
void QMC5883LCompass::read(){
	swWire.beginTransmission(_ADDR);
	swWire.write(0x00);
	int err = swWire.endTransmission();
	if (!err) {
		swWire.requestFrom(_ADDR, (byte)6);
		_vRaw[0] = (int)(int16_t)(swWire.read() | swWire.read() << 8);
		_vRaw[1] = (int)(int16_t)(swWire.read() | swWire.read() << 8);
		_vRaw[2] = (int)(int16_t)(swWire.read() | swWire.read() << 8);
		...
		//byte overflow = swWire.read() & 0x02;
		//return overflow << 2;
	}
}
